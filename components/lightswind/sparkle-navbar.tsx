...
type NavItem = {
  label: string;
  href: string;
  dropdown?: DropdownItem[]; // ✅ ADD THIS
};
  ...
  return (
    <>
      <style>{`
        ...
        .navigation-menu ul li:not(.active):hover {
      ...
      <nav className="navigation-menu" ref={navRef}>
        <ul>
          {items.map((item) => (
            ...
            >
              <a href={item.href} className="px-4 py-2 flex items-center gap-1">
                {item.label}
                {/* Add chevron for 'services' */}
              ...
              {item.dropdown && openDropdown === item.label && (
                <div className="absolute top-full left-0 mt-2 w-[700px] bg-black text-white rounded-lg shadow-lg z-50 p-4">
                  <div className="grid grid-cols-4 gap-4">
                    {item.dropdown.map((subItem: any, idx: number) => (
                      <a
                        key={subItem.name}
                        href={subItem.href}
                        className="flex flex-col items-start gap-2 px-3 py-4 rounded-lg hover:bg-gray-800 transition"
                      >
                        <span className="inline-flex items-center justify-center w-8 h-8 mb-1 text-lg">
                          {subItem.icon ? (
                            subItem.icon
                          ) : (
                        ...
                        <span className="font-semibold text-white text-sm">{subItem.name}</span>
                        {subItem.subtext && (
                          <span className="text-cyan-200 text-xs mb-1">{subItem.subtext}</span>
                        )}
                        {/* About/description for each service, from the image */}
                        {/* {subItem.name === 'AI & Intelligent Systems' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Automation, ML & LLM solutions</span>
                        )}
                        {subItem.name === 'Web Development' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Custom, scalable web apps</span>
                        )}
                        {subItem.name === 'App Development' && (
                          <span className="text-white text-xs opacity-80 leading-snug">iOS & Android native solutions</span>
                        )}
                        {subItem.name === 'Software Development' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Tailored enterprise solutions</span>
                        )}
                        {subItem.name === 'UI/UX Design' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Designs that convert users</span>
                        )}
                        {subItem.name === 'Graphic Design' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Creative branding visuals</span>
                        )}
                        {subItem.name === 'Digital Marketing' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Boost your brand visibility</span>
                        )}
                        {subItem.name === 'Research & Analytics' && (
                          <span className="text-white text-xs opacity-80 leading-snug">Data-driven insights</span>
                        )} */}
                      </a>
                    ))}
                  </div>
