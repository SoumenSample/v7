                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                      {(project.tasks || []).map((task) => (
                        <div key={task._id || task.id || task.title} className="space-y-2 rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-zinc-900/50 p-3">
                          <label className="flex items-start gap-3">
                            <input
                              ...
                              className="mt-1 h-4 w-4 rounded border-gray-400 dark:border-white/30 bg-white dark:bg-zinc-900/50 accent-gray-900 dark:accent-white"
                            />
                              ...
                              {task.subtasks.map((subtask, subtaskIndex) => (
                                <label key={subtask._id || subtask.id || `${task._id || task.id}-subtask-${subtaskIndex}`} className="flex items-center gap-3 text-xs text-gray-600 dark:text-white/75">
                                  <input
                                    ...
                                    className="h-4 w-4 rounded border-gray-400 dark:border-white/30 bg-white dark:bg-zinc-900/50 accent-gray-900 dark:accent-white"
                                  />
                ...
                <div className="min-w-[1200px] space-y-3">
                  <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${timeline.days.length}, minmax(${dayWidth}px, ${dayWidth}px))` }}>
                    {timeline.days.map((day) => {
                      ...
                      return (
                        <div key={`done-${day.toISOString()}`} className="min-h-[88px] rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-zinc-900/50 text-gray-900 dark:text-white p-2">
                          <p className="text-[10px] uppercase tracking-[0.18em] text-gray-500 dark:text-white/45">Done {formatDay(day)}</p>
                          {doneItems.length ? (
                            <div className="mt-2 space-y-1.5">
                              {doneItems.map((item, index) => (
                                <div key={`${dayKey}-${item.type}-${index}`} className="rounded-lg border border-gray-300 dark:border-white/20 bg-white dark:bg-zinc-900/50 px-2 py-1 text-[11px] text-gray-700 dark:text-white/80">
